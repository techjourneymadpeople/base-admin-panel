...
<div class="space-y-1.5">
    ...
    <div class="relative rounded-2xl">
        ...
        >
            @if($placeholder)
                <option value="" {{ empty($currentValue) ? 'selected' : '' }} disabled>{{ $placeholder }}</option>
            @endif
            ...
            @foreach($options as $key => $display)
                ...
                @endphp
                <option value="{{ $optValue }}" {{ (string)$currentValue === (string)$optValue ? 'selected' : '' }}>
                    {{ $optLabel }}
                </option>
            @endforeach
        ...
        <div class="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none text-[#428e75]">
