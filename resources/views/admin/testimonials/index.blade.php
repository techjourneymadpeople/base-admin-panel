<x-layouts.admin title="Kelola Testimonial">
    <x-admin.breadcrumb 
        title="Testimonial" 
        :items="[
            'Konten' => '',
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="p-5 rounded-3xl bg-white border border-[#99cab7]/30 shadow-2xs flex items-center justify-between">
                <div>
                    <span class="text-[11px] font-bold text-stone-400 uppercase tracking-wider">Total Testimoni</span>
                    <h3 class="text-2xl font-black text-[#1d3e35] mt-1">{{ number_format($stats['total']) }}</h3>
                    <span class="text-[10px] text-stone-400">Ulasan terdaftar</span>
                </div>
            ...
            <div class="p-5 rounded-3xl bg-white border border-[#99cab7]/30 shadow-2xs flex items-center justify-between">
                <div>
                    <span class="text-[11px] font-bold text-stone-400 uppercase tracking-wider">Rata-rata Rating</span>
                    <h3 class="text-2xl font-black text-amber-600 mt-1">{{ $stats['avg_rating'] }} <span class="text-sm font-semibold text-stone-400">/ 5.0</span></h3>
                    <span class="text-[10px] text-amber-600 font-semibold">Kepuasan Klien</span>
                </div>
            ...
            <div class="p-5 rounded-3xl bg-white border border-[#99cab7]/30 shadow-2xs flex items-center justify-between">
                <div>
                    <span class="text-[11px] font-bold text-stone-400 uppercase tracking-wider">Kategori Ulasan</span>
        ...
        <!-- Main Card: DataTable Testimonials -->
        <x-admin.card 
            title="Daftar Testimonial & Ulasan Klien" 
            subtitle="Kelola ulasan kepuasan pelanggan, testimoni mitra, dan rating bintang untuk website."
        >
            <x-slot:actions>
            ...
            <div class="overflow-x-auto w-full">
                <table id="testimonialsTable" class="w-full text-left text-xs border-collapse">
                    <thead>
                        <tr class="border-b border-stone-200 text-[#295c4d] font-bold uppercase tracking-wider text-[11px] bg-[#f2f8f5]/30">
                            ...
                            <th class="py-3 px-4">Pemberi Ulasan</th>
                            <th class="py-3 px-3">Rating</th>
                            <th class="py-3 px-4">Isi Testimoni</th>
    ...
    <script>
        (function() {
            function initTestimonialsTable() {
                if (typeof window.$ === 'undefined' || typeof window.$.fn.DataTable === 'undefined') {
                    setTimeout(initTestimonialsTable, 50);
                    return;
                }
                ...
                const table = $('#testimonialsTable').DataTable({
                    ...
                    columns: [
                        ...
                        { data: 'content_snippet', name: 'content', orderable: false, searchable: true },
                    ...
                    language: {
                        search: "_INPUT_",
                        searchPlaceholder: "Cari testimoni atau nama klien...",
                        lengthMenu: "Tampilkan _MENU_ data",
                        info: "Menampilkan _START_ - _END_ dari _TOTAL_ ulasan",
                        infoEmpty: "Belum ada testimoni",
                        zeroRecords: "Tidak ada ulasan yang cocok",
                        paginate: {
                            first: "«",
                            last: "»",
                            next: "›",
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', initTestimonialsTable);
            } else {
                initTestimonialsTable();
            }
        })();

        window.toggleTestimonialStatus = function(toggleUrl, checkbox) {
        ...
        window.confirmDeleteTestimonial = function(deleteUrl, name) {
