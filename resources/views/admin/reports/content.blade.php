<x-layouts.admin title="Laporan Produksi Konten & Kinerja Tim">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 {{ $config->article_module_enabled ? 'lg:grid-cols-4' : 'lg:grid-cols-3' }} gap-4 mb-6">
        <!-- Stat 1: Total Konten Keseluruhan -->
        <x-admin.stat-card
            title="Total Konten Tersimpan"
            :value="number_format($totalAllContent)"
            icon="layers"
            description="Akumulasi seluruh modul konten aktif"
        />
        ...
        <!-- Stat 4: Total Pembaca / Views (Tampil jika modul artikel aktif) -->
        @if($config->article_module_enabled)
            <x-admin.stat-card
    ...
    <div class="flex flex-wrap gap-3 mb-6">
        @if($config->article_module_enabled)
            <div class="p-3.5 rounded-2xl bg-white border border-[#99cab7]/30 shadow-2xs flex items-center gap-3 min-w-[150px] flex-1">
        ...
        <div class="lg:col-span-8 bg-white rounded-3xl p-6 border border-[#99cab7]/30 shadow-2xs">
            <div class="flex items-center justify-between gap-4 mb-6">
                <div>
                    ...
                    <p class="text-xs text-stone-500 mt-0.5">Statistik kuantitas konten yang diproduksi oleh tim setiap bulan untuk modul yang aktif.</p>
                </div>
        ...
        <div class="lg:col-span-4 bg-white rounded-3xl p-6 border border-[#99cab7]/30 shadow-2xs flex flex-col justify-between">
            <div class="flex items-center justify-between gap-4 mb-4">
                <div>
                    ...
                    <p class="text-xs text-stone-500 mt-0.5">Distribusi modul konten yang aktif.</p>
                </div>
                <div class="w-8 h-8 rounded-xl bg-[#31725e]/10 text-[#31725e] flex items-center justify-center shrink-0">
            ...
            <div class="pt-4 mt-2 border-t border-stone-100 grid grid-cols-2 gap-2 text-center text-xs">
                <div class="p-2 rounded-xl bg-stone-50 border border-stone-100">
                    <span class="text-stone-400 block text-[10px] font-bold">Modul Aktif</span>
                    <strong class="text-[#1d3e35] text-xs">{{ count($distributionData['labels']) }} Modul</strong>
                </div>
    ...
    @if($config->article_module_enabled)
        <div class="mb-6">
            <x-admin.card
                title="Kinerja & Aktivitas Tim Kontributor Artikel (Admin, Editor, & Staf)"
                subtitle="Ringkasan hasil kerja pembuatan artikel, tulisan terbit/draft, dan tayangan views dari masing-masing akun staf."
            >
    ...
    <div>
        <x-admin.card
            ...
            subtitle="Daftar konten terbaru yang diinputkan oleh tim di seluruh modul website yang aktif."
        >
            <div class="overflow-x-auto">
                <table class="w-full text-left text-xs border-collapse">
                    <thead>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="py-6 text-center text-stone-400 italic">
                                    Belum ada konten yang dibuat pada modul yang aktif saat ini.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                new window.Chart(distCtx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: @json($distributionData['colors']),
                            borderWidth: 2,
                            borderColor: '#ffffff',
                            hoverOffset: 4
                        }]
