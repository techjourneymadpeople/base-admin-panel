<x-layouts.admin title="Edit Artikel: {{ $article->title }}">
    <x-admin.breadcrumb 
        title="Edit Artikel" 
        :items="[
            'Article SEO' => '',
    ...
    <div class="space-y-6" x-data="articleEditFormHandler()">
                    ...
                    >
                        <div class="space-y-5">
                            ...
                            <div class="space-y-1.5">
                                ...
                                >
                                    @foreach($tags as $tag)
                                        <option value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $selectedTagIds)) ? 'selected' : '' }}>
                                            {{ $tag->name }}
                                        </option>
                                    @endforeach
                                </select>
    ...
    <script>
        ...
        function articleEditFormHandler() {
            return {
                ...
                excerpt: "{{ old('excerpt', addslashes($article->excerpt ?? '')) }}",
                metaTitle: "{{ old('meta_title', addslashes($article->meta_title ?? '')) }}",
                metaDescription: "{{ old('meta_description', addslashes($article->meta_description ?? '')) }}",
                thumbnailMediaId: "{{ old('thumbnail_media_id', $article->thumbnail_media_id ?? '') }}",
                thumbnailUrl: "{{ old('thumbnail_url', $article->getThumbnail() ?? '') }}",
