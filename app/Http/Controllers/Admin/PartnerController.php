        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($partners)
                ...
                ->addColumn('partner_info', function (Partner $partner) {
                    ...
                    $logoHtml = $logo 
                        ? '<div class="w-12 h-12 rounded-xl bg-white border border-stone-200 p-1 flex items-center justify-center shrink-0 shadow-2xs overflow-hidden"><img src="' . e($logo) . '" alt="' . e($partner->name) . '" class="w-full h-full object-contain"></div>'
                        : '<div class="w-12 h-12 rounded-xl bg-stone-100 border border-stone-200 flex items-center justify-center text-stone-400 shrink-0 font-black text-sm uppercase">' . substr($partner->name, 0, 2) . '</div>';

                    $websiteHtml = $partner->website_url 
                        ? '<a href="' . e($partner->website_url) . '" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 text-[11px] text-[#31725e] hover:underline font-semibold mt-0.5"><i data-lucide="external-link" class="w-3 h-3"></i> ' . e(parse_url($partner->website_url, PHP_URL_HOST) ?: $partner->website_url) . '</a>'
                        : '<span class="text-[11px] text-stone-400 italic">Tidak ada website</span>';
                ...
                ->addColumn('category_badge', function (Partner $partner) {
                    $cat = $partner->category ?: 'Mitra Strategis';
