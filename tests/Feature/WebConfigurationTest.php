...
class WebConfigurationTest extends TestCase
{
    ...
    public function test_gallery_module_disabled_blocks_access_to_gallery_routes(): void
    {
        $this->seed(RoleAndPermissionSeeder::class);
        ...
        $config = WebConfiguration::current();
        $config->update(['gallery_module_enabled' => false]);

        // 1. Web request redirects to dashboard
        $response = $this->actingAs($admin)->get(route('admin.gallery-activities.index'));
        $response->assertRedirect(route('admin.dashboard'));
        $response->assertSessionHas('error');

        // 2. AJAX request returns 403 JSON
        $ajaxResponse = $this->actingAs($admin)->getJson(route('admin.gallery-activities.index'), [
            'X-Requested-With' => 'XMLHttpRequest',
        ]);
        $ajaxResponse->assertStatus(403);
    }

    public function test_gallery_module_enabled_allows_access_to_gallery_routes(): void
    {
        $this->seed(RoleAndPermissionSeeder::class);
        ...
        $config = WebConfiguration::current();
        $config->update(['gallery_module_enabled' => true]);
